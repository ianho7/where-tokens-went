...
function renderKpis(result, locale) {
    ...
    const sessionBreakdown = result.summary.topLevelSessionCount.value !== null && result.summary.subagentSessionCount.value !== null
        ? [[labels.topLevelSessions, result.summary.topLevelSessionCount], [labels.subagentSessions, result.summary.subagentSessionCount]]
        : [];
    const items = [
        ...
        ...sessionBreakdown,
        ...(result.summary.reportedCost.value === null ? [] : [[labels.reportedCost, result.summary.reportedCost]]),
        ...(typeof result.report.apiEquivalentCost.coveragePercent.value === "number" && result.report.apiEquivalentCost.coveragePercent.value >= 80 && result.report.apiEquivalentCost.total.value !== null ? [[locale === "zh-CN" ? "API 等价估算（USD）" : "API-equivalent estimate (USD)", result.report.apiEquivalentCost.total]] : []),
    ];
