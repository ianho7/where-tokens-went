...
function localizeWarningEnglish(warning) {
    ...
    if (unsupported)
        return unsupported[1] + " Codex Session" + (unsupported[1] === "1" ? "" : "s") + " contain unsupported accounting records; the audit may be partial.";
    ...
    if (missingTime)
        return missingTime[1] + " Codex Session" + (missingTime[1] === "1" ? "" : "s") + " contain records without usable timestamps; only time-scoped records were analysed.";
    return warning;
}
function localizeWarningChinese(warning) {
    const unsupported = /^(\d+) Codex Session(?:s)? contain(?:s)? unsupported accounting records; only a partial audit is reported\.$/.exec(warning);
    if (unsupported)
        return unsupported[1] + " 个 Codex Session 包含本报告暂时无法解析的用量记录，报告可能不完整。";
    const missingTime = /^(\d+) Codex Session(?:s)? contain(?:s)? accounting records without a usable timestamp; only time-scoped records were analysed\.$/.exec(warning);
    if (missingTime)
        return missingTime[1] + " 个 Codex Session 包含无法使用的时间戳；只统计时间范围明确的记录。";
    return warning;
}
