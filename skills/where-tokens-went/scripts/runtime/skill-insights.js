...
function selectSkillCandidates(skills, totalTasks) {
    ...
    const associatedSessionCandidates = activeSkills
        ...
        .filter(({ metrics }) => metrics.associatedSessionTokens !== null &&
        metrics.tasks > 0 &&
        ...
        .sort((left, right) => (right.metrics.associatedSessionTokens ?? 0) - (left.metrics.associatedSessionTokens ?? 0) ||
...
function hasUnqualifiedTokenScaleRelation(text) {
    ...
    return clauses.some((clause) => {
        ...
        while ((match = relationPattern.exec(clause)) !== null) {
            ...
            if (!TOKEN_SCALE_BOUNDARY_REGEX.test(before) && !TOKEN_SCALE_BOUNDARY_REGEX.test(after))
                return true;
        }
        return false;
    });
}
function hasUnsupportedCausalAssertion(text) {
    const normalized = normalizeText(text);
    if (DIRECT_REPEATED_INJECTION_ASSERTION_REGEX.test(normalized))
        return true;
...
function validateSkillInsights(raw, snapshot, globalUsage) {
    ...
    for (const item of rawList) {
        ...
        const hasAssociatedSessionEvidence = validEvidence.some((entry) => entry.kind === "skill_metric" &&
            entry.metric === "associatedSessionTokens" &&
            typeof entry.value === "number" &&
            Number.isFinite(entry.value));
        const hasAssociatedSessionBaseline = validEvidence.some((entry) => entry.kind === "global_metric" &&
