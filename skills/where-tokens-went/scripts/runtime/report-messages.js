...
exports.REPORT_MESSAGES = void 0;
exports.reportMessagesFor = reportMessagesFor;
function localizeWarningEnglish(warning) {
    ...
    if (unsupported)
        return unsupported[1] + " Codex task" + (unsupported[1] === "1" ? "" : "s") + " contain records that could not be parsed; the audit may be partial.";
    ...
    if (missingTime)
        return missingTime[1] + " Codex task" + (missingTime[1] === "1" ? "" : "s") + " contain records without usable timestamps; only records inside the requested time range were analysed.";
    if (warning === "A Codex rollout could not be read and was skipped.")
        return "A Codex history record could not be read and was skipped.";
    if (warning === "A Claude Code transcript could not be read and was skipped.")
        return "A Claude Code history record could not be read and was skipped.";
    if (/^No Codex rollout history was found/.test(warning))
        return "No Codex history was found in the selected range.";
    if (/^No Claude Code transcript history was found/.test(warning))
        return "No Claude Code history was found in the selected range.";
    return /[A-Za-z]/.test(warning) ? "A source record could not be read and was skipped." : warning;
}
function localizeWarningChinese(warning) {
    if (warning === "Some Codex Sessions have Turn snapshots that do not reconcile to their per-response Usage; only individually reconciled Sessions are eligible for AI analysis.") {
        return "部分 Codex 任务的轮次快照与每次模型调用的 Token 记录暂时无法核对；只有已核对的任务才会进入 AI 解读。";
    }
    const unsupported = /^(\d+) Codex Session(?:s)? contain(?:s)? unsupported accounting records; only a partial audit is reported\.$/.exec(warning);
    if (unsupported)
        return unsupported[1] + " 个 Codex 任务包含暂时无法解析的 Token 记录，数据可能不完整。";
    const missingTime = /^(\d+) Codex Session(?:s)? contain(?:s)? accounting records without a usable timestamp; only time-scoped records were analysed\.$/.exec(warning);
    if (missingTime)
        return missingTime[1] + " 个 Codex 任务缺少可用时间戳；只统计时间范围明确的记录。";
    if (warning === "A Codex rollout could not be read and was skipped.")
        return "有一份 Codex 历史记录无法读取，已跳过。";
    if (warning === "A Claude Code transcript could not be read and was skipped.")
        return "有一份 Claude Code 历史记录无法读取，已跳过。";
    if (/^No Codex rollout history was found/.test(warning))
        return "所选范围内没有找到 Codex 历史记录。";
    if (/^No Claude Code transcript history was found/.test(warning))
        return "所选范围内没有找到 Claude Code 历史记录。";
    return /[A-Za-z]/.test(warning) ? "有一条数据记录无法读取，已跳过。" : warning;
}
function localizeLimitationChinese(limitation) {
    ...
    if (limitation === "no ModelCall had a complete mutually exclusive Token composition for cache coverage")
        return "没有模型调用提供完整且不重叠的 Token 构成，因此无法计算缓存构成可分解比例";
    ...
    if (limitation === "the all-uncached comparison requires at least one selected ModelCall with compatible exact pricing")
        return "要比较不缓存情况下的成本，至少需要一个价格信息完整且匹配的模型调用";
    if (limitation === "currency requires at least one selected ModelCall with an exact Provider/model match and compatible non-zero price dimensions")
        return "要计算金额，至少需要一个来源、模型和非零价格档位都精确匹配的模型调用";
...
function localizeMethodChinese(method) {
    ...
    if (method.startsWith("median of earliest valid ModelCall Token totals in "))
        return method.replace(/^median of earliest valid ModelCall Token totals in (.+)$/, "取 $1 中每个最早有效模型调用的 Token 总量中位数");
    return "按所选历史记录和支持字段计算";
}
function localizeLimitationEnglish(limitation) {
    const exact = {
    ...
    const missingSkillAssociation = /^no source-proven ModelCall association was available for (.+)$/.exec(limitation);
    if (missingSkillAssociation)
        return "There is not enough evidence to associate model calls with " + missingSkillAssociation[1] + ".";
    return limitation
...
function localizeMethodEnglish(method) {
    if (!method)
        return "unavailable";
    const exact = {
    ...
    if (method.startsWith("partial all-uncached counterfactual"))
        return "The all-uncached estimate prices ordinary input, cache reads, and cache writes as ordinary input; output pricing is unchanged and unpriced usage is excluded.";
    if (method.startsWith("all-uncached counterfactual"))
        return "The all-uncached estimate prices ordinary input, cache reads, and cache writes as ordinary input; output pricing is unchanged.";
    if (method.startsWith("all-uncached API-equivalent estimate minus observed API-equivalent estimate"))
        return "All-uncached API-equivalent estimate minus observed API-equivalent estimate; a positive value indicates lower estimated cost with caching.";
    if (method.startsWith("cache savings divided by all-uncached API-equivalent cost"))
        return "Estimated savings divided by the all-uncached API-equivalent estimate and rounded to two decimals.";
    if (method.startsWith("cost difference divided by all-uncached API-equivalent estimate"))
        return "Cost difference divided by the all-uncached API-equivalent estimate and rounded to two decimals.";
    if (method.startsWith("sum of compatible "))
        return method.replace(/^sum of compatible (.+) Token buckets$/, "Sum of compatible $1 Token buckets.");
    if (method.startsWith("median of earliest valid ModelCall Token totals in "))
        return method.replace(/^median of earliest valid ModelCall Token totals in (.+)$/, "Median Token total of the earliest valid model call in each $1.");
    ...
    keySession: {
        ...
        unavailableReason: (reason) => {
            ...
            if (reason.includes("Codex Token accounting"))
                return "The task's Token records could not be reconciled.";
            if (reason.includes("Audit fingerprint"))
                return "The explanation does not match the current usage record.";
            if (reason.includes("Turn outside"))
                return "The explanation refers to a round outside this task.";
            if (reason.includes("primaryFinding"))
                return "The core judgment did not pass the evidence check.";
            if (reason.includes("recommendation"))
                return "The proposed action did not pass the evidence check.";
            if (reason.includes("evidenceRead"))
                return "The explanation does not identify the rounds it used.";
...
const ZH = {
    ...
    keySession: {
        ...
        unavailableReason: (reason) => {
            ...
            if (reason.includes("Codex Token accounting"))
                return "Codex Token 记录暂时无法核对。";
            if (reason.includes("Audit fingerprint"))
                return "任务解读与当前统计不匹配。";
            if (reason.includes("Turn outside"))
                return "任务解读引用了当前任务之外的轮次。";
            if (reason.includes("primaryFinding"))
...
exports.REPORT_MESSAGES = { "en-US": EN, "zh-CN": ZH };
function reportMessagesFor(locale) {
    return exports.REPORT_MESSAGES[locale];
}
