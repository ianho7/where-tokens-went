...
For Codex, a non-null Key Session conclusion is accepted only when that selected task's `keySessionTokenAccounting` status is `reconciled`. A global `tokenAccountingStatus: mismatch` does not block independently reconciled tasks. A selected task with `mismatch` or `unavailable` accounting remains eligible for a grounded task context and explicit null analysis, but not a guessed mechanism or action; do not discard all selected analyses because one task is not reconciled.
