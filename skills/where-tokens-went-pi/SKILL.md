---
name: where-tokens-went-pi
description: Explain recent Pi usage from local Session JSONL using the where-tokens-went deterministic tool.
---

# where-tokens-went for Pi
...
```text
node <skill-directory>/scripts/where-tokens-went.js inspect --harness pi --cwd <absolute-current-project-path> --since <duration> --format json
```
