...
function scoreHtml(html) {
  ...
  const checks = [
    ...
    check('charts', 'Chart palette and independent series', 8, /#1b365d/.test(boot) && /areaStyle:\{color,opacity:\.18\}/.test(boot) && !/stack:'tokens'/.test(boot), 'brand chart color, flat area tint, no token stacking', !hasBoot),
