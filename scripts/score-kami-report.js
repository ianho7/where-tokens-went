...
function scoreHtml(html) {
  ...
  const checks = [
    ...
    check('surfaces', 'Quiet Kami surfaces without effects', 6, /\.quiet-callout\{[^}]*background:var\(--ivory\);border:0/.test(style) && !/linear-gradient|box-shadow/.test(style), 'ivory quiet callout; no gradients or shadows', !hasStyle),
    ...
    check('charts', 'Kami contrast ladder and redundant series encoding', 8, /#1b365d/.test(boot) && /#2d5a8a/.test(boot) && /#6b6a64/.test(boot) && /lineType,symbol,focus/.test(boot) && /areaStyle:\{color,opacity:\.1\}/.test(boot) && /areaStyle:\{color,opacity:\.12\}/.test(boot) && !/areaStyle:\{color,opacity:\.18\}/.test(boot) && !/stack:'tokens'/.test(boot), 'official ladder, line/point encodings, focal-only area fill, no token stacking', !hasBoot),
