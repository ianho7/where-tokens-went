...
报告默认使用内置字体。需要更换字体时，可以在对话中直接提供本地 `.ttf`、`.otf`、`.woff` 或 `.woff2` 文件，或在最终报告命令中使用 `--font <font-file>`；可用 `--font-family <name>` 指定 CSS 字体名。仅提供系统字体名无法用于独立报告；当前一个自定义字体文件会同时作为报告的正文和标题字体。最终 HTML 会只嵌入完成报告文字所需的 WOFF2 子集；配置文件不可读时回到内置字体，子集化失败时保留可读的完整嵌入字体并继续生成报告。
