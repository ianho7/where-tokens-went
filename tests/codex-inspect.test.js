...
const { renderHtml, renderText, renderShare, resolveReportProjectName, collectReportFontCharacters, subsetReportFonts } = require('../dist/src/report.js');
...
test('CLI uses LiteLLM pricing by default and accepts the explicit pricing flag', () => {
  ...
  const fontOptions = parseArgs(['inspect', '--harness', 'codex', '--cwd', 'D:\\project', '--font', 'C:\\fonts\\report.ttf', '--font-family', '宋体']);
  assert.equal(fontOptions.fontPath, 'C:\\fonts\\report.ttf');
  assert.equal(fontOptions.fontFamily, '宋体');
  assert.throws(() => parseArgs(['inspect', '--harness', 'codex', '--cwd', 'D:\\project', '--font-family', '宋体']), /requires --font/);
  assert.throws(() => parseArgs(['inspect', '--harness', 'codex', '--cwd', 'D:\\project', '--pricing', 'local']), /must be litellm/);
});

test('report fonts can be configured and are subsetted to WOFF2 after HTML composition', async () => {
  const fontPath = path.resolve(__dirname, '..', 'assets', 'fonts', 'TsangerJinKai02-W04.ttf');
  const result = analyseAudit(
    { cwd: 'D:\\project', allProjects: false, since: new Date('2026-09-01T00:00:00.000Z') },
    {
      sessions: [{ harness: 'codex', sessionId: 'font-session', title: '字体测试', projectCwd: 'D:\\project', startedAt: '2026-09-08T08:00:00.000Z', endedAt: '2026-09-08T08:00:00.000Z', parentSessionId: null, sourceVersion: null }],
      modelCalls: [{ sessionId: 'font-session', callId: 'font-call', timestamp: '2026-09-08T08:00:00.000Z', provider: 'openai', model: 'gpt-5', inputTokens: 90, cachedInputTokens: 0, cacheWriteTokens: 0, outputTokens: 10, reasoningTokens: 0, totalTokens: 100, reportedCost: null, status: 'ok', tokenProvenance: 'reported' }],
      toolCalls: [],
      lifecycle: [],
  ...
  const full = renderHtml(result, 'zh-CN', undefined, undefined, { filePath: fontPath, family: 'Configured Report Font' });
  assert.match(full, /font-family:"Configured Report Font"/);
  assert.match(full, /data:font\/ttf;base64,/);
  assert.match(collectReportFontCharacters(full), /字体测试/);

  const optimized = await subsetReportFonts(full);
  const fonts = [...optimized.matchAll(/data:font\/woff2;base64,([A-Za-z0-9+/=]+)/g)];
  assert.equal(fonts.length, 2);
  assert.doesNotMatch(optimized, /data:font\/ttf;base64,/);
  assert.equal(optimized.match(/format\("woff2"\)/g)?.length, 2);
  assert.ok(optimized.length < full.length);
  const normalizeFontData = (html) => html
    .replace(/data:font\/(?:ttf|otf|woff|woff2);base64,[A-Za-z0-9+/=]+/g, 'FONT_DATA')
    .replace(/format\("(?:truetype|opentype|woff|woff2)"\)/g, 'format("FONT_FORMAT")');
  assert.equal(normalizeFontData(optimized), normalizeFontData(full));
  for (const font of fonts) assert.equal(Buffer.from(font[1], 'base64').subarray(0, 4).toString('ascii'), 'wOF2');
  assert.match(optimized, /字体测试/);

  const broken = full.replace(/data:font\/ttf;base64,[A-Za-z0-9+/=]+/, 'data:font/ttf;base64,AAAA');
  assert.equal(await subsetReportFonts(broken), broken);
});
  ...
  try {
    ...
    assert.equal(audit.turns.length, 1);
      ...
      const synthesis = {
        ...
        {
          title: '模型调用与单次上下文规模呈现高度集中',
          analysis: '用量主要由少数高输入模型调用贡献，其余调用的消耗相对有限。',
          evidenceRefs: ['summary:modelCallCount'],
          support: 'moderate',
          uncertainty: '调用分布仅反映当前审计周期内的活动模式。',
        },
      ],
    ...
    for (const harness of ['codex', 'claude']) {
      ...
      await runBundledWithInput(harness, ['compose-report', '--locale', 'zh-CN', '--html', htmlPath, '--font', path.resolve(__dirname, '..', 'assets', 'fonts', 'TsangerJinKai02-W04.ttf'), '--font-family', 'Composed Report Font'], payload, env);
      ...
      assert.match(html, /font-family:"Composed Report Font"/);
...
test('Codex Skill path reports a deterministic long-session check without raw content', async () => {
  ...
  const project = path.join(root, 'project');
  const codexHome = path.join(root, 'codex-home');
  const sessions = path.join(codexHome, 'sessions', ...isoHoursAgo(2).slice(0, 10).split('-'));
  await mkdir(project, { recursive: true });
  await mkdir(sessions, { recursive: true });
