...
### Codex Rollout 读取性能与数据剪枝约定

针对大规模历史目录（数百至数千个 rollout 文件），Codex Reader 必须遵循以下 IO 剪枝与定位规则：

1. **日期目录剪枝 (YYYY/MM/DD Pruning)**：当指定 --since <duration>（如 7d）时，计算出的起始时间所对应的日期确定最早目标目录。在遍历 $CODEX_HOME/sessions 时，在目录层级比对 YYYY/MM/DD，直接跳过早于 since 日期的目录分支，避免对数月前的历史文件做无谓的文件系统枚举与状态检查。
2. **首行预检与快速跳过 (cwd Early Check)**：对于符合日期范围的 rollout JSONL 文件，当按项目过滤 (--cwd) 时，优先仅读取首行（包含 SessionMeta，携带 cwd 字段）；若 cwd 与目标项目不匹配，立即关闭文件并跳过，避免读取解析数十 MB 的无关内容。
3. **保留 Session 相对路径用于内容证据定向读取**：初次历史采集过程中，记录每个 Session 对应的相对文件路径；后续内容证据提取阶段直接依据该路径进行定向读取，将内容证据读取从 O(N) 全盘重扫优化为 O(1) 定向单文件读取。
