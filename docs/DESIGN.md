...
- **Dual-Track DAG AI Concurrency**: Report synthesis and key session analysis must be orchestrated as a directed acyclic graph with concurrency capped at 2 (AI_CONCURRENCY = 2). Lane 1 (report-synthesis) dispatches as soon as audit.json is ready, while Lane 2 (content-read -> key-session-analysis) dispatches in parallel, collapsing sequential inference latency down to max(T_synth, T_read + T_key).
