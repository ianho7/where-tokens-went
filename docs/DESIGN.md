...
The standalone HTML adds a presentation-only chapter layer above that unchanged module hierarchy. It preserves the current module order and groups the existing modules into five stable reading stages: overview contains Scope, Coverage, and Usage Overview; diagnosis contains Findings, Cache Economics, First-request Burden, and Skill Evidence; patterns contains the optional week comparison followed by time, models, tools, and Session ranking; trace contains Key Session Analysis; caveats contains limitations and the privacy/method footer. Each stage begins with one visible Kami-style Section marker. The localized marker contract is `01 · 概览`, `02 · 诊断`, `03 · 模式`, `04 · 追踪`, `05 · 限制` for Chinese and `01 · Orient`, `02 · Diagnose`, `03 · Patterns`, `04 · Trace`, `05 · Caveats` for English. Numbers are stable across reports, start at one, and have no total-count denominator. The renderer does not add a progress bar, table of contents, navigation behavior, or new container treatment; existing headings, content, charts, tables, controls, spacing within modules, accessible names, and Evidence semantics remain unchanged. Fixed chapter labels belong to the typed presentation localization table, not `AuditResult` or Host Agent prose.
