...
- Five localized narrative chapter markers make the existing standalone-HTML module sequence explicit without reordering or redesigning those modules: Chinese uses `01 · 概览`, `02 · 诊断`, `03 · 模式`, `04 · 追踪`, and `05 · 限制`; English uses `01 · Orient`, `02 · Diagnose`, `03 · Patterns`, `04 · Trace`, and `05 · Caveats`. The markers use Kami's additive Section-number grammar, start at one, and do not add a total-count denominator, progress indicator, or navigation UI.
...
- the standalone HTML shows exactly five localized chapter markers in `01` through `05` order; they group the unchanged modules as overview (Scope, Coverage, and Usage Overview), diagnosis (Findings, Cache Economics, First-request Burden, and Skill Evidence), patterns (week comparison when available, time, models, tools, and Session ranking), trace (Key Session Analysis), and caveats (limitations plus privacy/method), without a `/ 05` denominator or any change to module order, content, interaction, or Evidence semantics;
