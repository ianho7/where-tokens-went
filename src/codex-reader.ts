...
export async function readCodex(scope: ReadScope): Promise<ReadResult> {
  ...
  for (const pending of pendingById.values()) {
    ...
    if (calls.length === 0 || calls.some((call) => call.totalTokens === null)) responseComplete = false;
    else responseTotal += calls.reduce((sum, call) => sum + call.totalTokens!, 0);
    if (pending.turnTokenSnapshots.size === 0 || [...pending.turnTokenSnapshots.values()].some((usage) => usageTotal(usage) === null)) turnComplete = false;
    else turnTotal += [...pending.turnTokenSnapshots.values()].reduce((sum, usage) => sum + usageTotal(usage)!, 0);
    if (usageTotal(pending.threadTokenSnapshot) === null) threadComplete = false;
    else threadTotal += usageTotal(pending.threadTokenSnapshot)!;
  }

  const tokenAccounting: TokenAccountingSummary = selectedSessionCount === 0 || !responseComplete
  ...
  if (tokenAccounting.status === "mismatch") coverage.warnings.push("Codex response Usage did not reconcile with the latest cumulative Turn/thread snapshot; AI analysis is unavailable.");
