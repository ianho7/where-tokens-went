...
interface PendingSession {
  ...
  turns: TurnRecord[];
  turnTokenSnapshots: Map<string, UsageValues>;
  threadTokenSnapshot: UsageValues | null;
...
interface ResponseUsage {
  ...
  usage: UsageValues;
  turnId: string | null;
  turnTokenUsage: UsageValues | null;
  threadTokenUsage: UsageValues | null;
}
...
function usageTotal(usage: UsageValues | null): number | null {
  return usage?.totalTokens ?? null;
}

function completeUsageSnapshot(value: unknown): UsageValues | null {
...
async function rolloutFiles(root: string, scope?: ReadScope): Promise<string[]> {
  const found: string[] = [];
  let cutoffYear: string | undefined;
  let cutoffMonth: string | undefined;
  let cutoffDay: string | undefined;
  let untilYear: string | undefined;
  let untilMonth: string | undefined;
  let untilDay: string | undefined;
  ...
  async function visit(directory: string, relParts: string[]): Promise<void> {
    let entries;
    try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        if (relParts.length === 0 && /^\d{4}$/.test(entry.name)) {
          if (cutoffYear && entry.name < cutoffYear) continue;
          if (untilYear && entry.name > untilYear) continue;
        } else if (relParts.length === 1 && /^\d{2}$/.test(entry.name)) {
          const ym = `${relParts[0]}/${entry.name}`;
          if (cutoffYear && cutoffMonth && ym < `${cutoffYear}/${cutoffMonth}`) continue;
          if (untilYear && untilMonth && ym > `${untilYear}/${untilMonth}`) continue;
        } else if (relParts.length === 2 && /^\d{2}$/.test(entry.name)) {
          const ymd = `${relParts[0]}/${relParts[1]}/${entry.name}`;
          if (cutoffYear && cutoffMonth && cutoffDay && ymd < `${cutoffYear}/${cutoffMonth}/${cutoffDay}`) continue;
          if (untilYear && untilMonth && untilDay && ymd > `${untilYear}/${untilMonth}/${untilDay}`) continue;
        }
...
function responseUsage(record: JsonObject, payload: JsonObject): ResponseUsage | null {
  const event = firstObject(
    ...
    isType(record, payload, "token_usage_record") ? payload : null,
    payload.event,
    record.event,
  );
...
function ensureTurn(pending: PendingSession, turnId: string | null, timestamp: string | null): TurnRecord | null {
  if (!turnId) return null;
  const existing = pending.turns.find((turn) => turn.turnId === turnId);
  if (existing) {
    if (timestamp && (!existing.startedAt || Date.parse(existing.startedAt) > Date.parse(timestamp))) existing.startedAt = timestamp;
    return existing;
  }
...
function updateTurnFromTask(pending: PendingSession, payload: JsonObject, timestamp: string | null, complete: boolean): void {
...
function appendLifecycle(pending: PendingSession, event: LifecycleRecord): void {
  const duplicate = pending.lifecycle.some((existing) =>
    existing.kind === event.kind &&
    existing.relatedId === event.relatedId &&
    existing.timestamp === event.timestamp &&
    existing.turnId === event.turnId,
  );
  if (!duplicate) pending.lifecycle.push(event);
}
...
export async function readCodex(scope: ReadScope): Promise<ReadResult> {
  ...
  const sessionIndex = useSessionIndex ? loadSessionIndex() : new Map<string, CachedFileParseResult>();
  ...
  for (const file of files) {
    ...
    for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
      ...
      if (eventType === "task_started") updateTurnFromTask(pending, payload, timestamp, false);
      if (eventType === "task_complete" || eventType === "task_completed") updateTurnFromTask(pending, payload, timestamp, true);
      ...
      if (response) {
        ...
        if (responseTurnId && response.turnTokenUsage) pending.turnTokenSnapshots.set(responseTurnId, response.turnTokenUsage);
        if (response.threadTokenUsage) pending.threadTokenSnapshot = response.threadTokenUsage;
        ...
        if (existingIndex >= 0) {
          ...
          if (callCompleteness > existingCompleteness) pending.rawCalls[existingIndex] = call;
        } else {
  ...
  const reconciledSessionIds: string[] = [];
  const mismatchedSessionIds: string[] = [];
  for (const pending of pendingById.values()) {
    ...
    for (const call of calls) {
      if (!inScope(call.timestamp, scope)) continue;
    ...
    const sessionTurnTotal = sessionTurnComplete
      ...
      : null;
    if (!sessionResponseComplete) responseComplete = false;
    else responseTotal += sessionResponseTotal!;
    if (!sessionTurnComplete) turnComplete = false;
    else turnTotal += sessionTurnTotal!;
    if (
      sessionResponseTotal !== null &&
  ...
  const tokenAccounting: TokenAccountingSummary = accountedSessionCount === 0 || !responseComplete
  ...
  if (mismatchedSessionIds.length > 0) coverage.warnings.push("Some Codex Sessions have Turn snapshots that do not reconcile to their per-response Usage; only individually reconciled Sessions are eligible for AI analysis.");
