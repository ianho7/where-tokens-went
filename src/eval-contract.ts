...
import type { CodexProvenanceResult } from "./codex-provenance";

export const EVAL_CONTRACT_VERSION = 1 as const;
export const PROMOTION_MINIMUM_TRIALS = 3;
export const PROMOTION_MINIMUM_IMPROVEMENT_DELTA = 0.05;

export type EvalLane = "report-synthesis" | "key-session-analysis" | "skill-insights";
export type EvalSplit = "smoke" | "regression" | "held-out";
export type EvalPrivacy = "redacted" | "local-only";
export type EvalCaseClass = "capability" | "regression";
export type EvalOutcome = "accepted" | "rejected" | "inconclusive" | "unavailable";
export type BudgetEvidencePolicy = "required" | "advisory";
export type TokenMetric = "output_tokens" | "total_tokens";
export type BudgetKind = "token" | "time";
export type BudgetObservation = "within" | "exceeded" | "unavailable";
...
export interface BudgetEvaluation {
  ...
  value: number | null;
  limit: number | null;
  policy: BudgetEvidencePolicy;
  outcome: "within" | "rejected" | "inconclusive" | "advisory";
  errorCode: string | null;
}

export interface EvalCase {
  version: typeof EVAL_CONTRACT_VERSION;
  id: string;
  lane: EvalLane;
  class: EvalCaseClass;
  split: EvalSplit;
...
export interface GenerationRecord {
  ...
  codexProvenance: CodexProvenanceResult;
...
export interface RoleExecutionRecord {
  version: typeof EVAL_CONTRACT_VERSION;
  kind: "host-agent-quality-grade" | "host-agent-optimizer" | "host-agent-reviewer";
  executionId: string;
  role: "quality-grader" | "optimizer" | "reviewer";
  contextId: string;
  host: string;
  outputHash: string;
  codexProvenance: CodexProvenanceResult;
...
export interface EvalTrial {
  ...
  contractStatus: "passed" | "blocked" | "unknown";
  quality: Record<string, number | "unknown">;
  qualityEvidence?: Record<string, string[]>;
...
export interface EvalBenchmarkSummary {
  ...
  dimensions: Record<string, { mean: number | null; stddev: number | null; unknownCount: number }>;
  durationMs: { mean: number | null; stddev: number | null };
  tokenCount: { mean: number | null; stddev: number | null };
  failures: string[];
}
...
export interface EvalExperiment {
  ...
  budgetEvidencePolicy: BudgetEvidencePolicy;
  tokenMetric: TokenMetric;
  caseIds: string[];
  modelConfig: EvalModelConfig;
  status: "smoke" | "regression" | "held-out" | "review" | "promoted" | "rejected" | "inconclusive";
  createdAt: string;
}
...
export interface EvalReview {
  ...
  timeBudgetPassed: boolean;
  blockingFailures: string[];
  candidateImproved: boolean;
  reviewer: string | null;
  approved: boolean;
  conclusion: "eligible" | "rejected" | "inconclusive";
  artifactPath: string;
  gateErrors: string[];
  reviewEvidenceHash: string | null;
  reviewArtifactHash: string | null;
  reviewerApprovalHash: string | null;
  qualityGraderRecordHash: string | null;
  optimizerRecordHash: string | null;
  tokenBudgetObservation: BudgetObservation;
  timeBudgetObservation: BudgetObservation;
...
export interface AcceptedBaseline {
  ...
  promptHashes: Record<EvalLane, string>;
  runtimeHash: string;
  modelConfig: EvalModelConfig;
  caseIds: string[];
  trials: EvalTrial[];
  knownVariance: Record<string, string>;
  budgetEvidencePolicy: BudgetEvidencePolicy;
  tokenMetric: TokenMetric;
  budgetLimitations: string[];
  privacy: "redacted";
...
export interface QualityGradeDimension {
...
export interface QualityGrade {
  blindId: string;
  dimensions: Record<string, QualityGradeDimension>;
  createdAt: string;
}
...
function isCodexProvenance(value: unknown): value is CodexProvenanceResult {
  if (!isRecord(value) || value.harness !== "codex" || typeof value.rolloutPath !== "string" || value.rolloutPath.length === 0 || !isHash(value.rolloutHash)
    ...
    || (value.tokenMetric !== "output_tokens" && value.tokenMetric !== "total_tokens") || (value.tokenValue !== null && typeof value.tokenValue !== "number") || !isHash(value.outputHash)) return false;
  if (value.usage !== null) {
    if (!isRecord(value.usage)) return false;
    const usage = value.usage;
    if (!["inputTokens", "outputTokens", "totalTokens"].every((key) => usage[key] === null || typeof usage[key] === "number")) return false;
  }
  return Date.parse(value.startedAt) <= Date.parse(value.endedAt);
}

function isLane(value: unknown): value is EvalLane {
  return value === "report-synthesis" || value === "key-session-analysis" || value === "skill-insights";
}

function isVersion(value: unknown): value is typeof EVAL_CONTRACT_VERSION {
  return value === EVAL_CONTRACT_VERSION;
}

export function isEvalCase(value: unknown): value is EvalCase {
  if (!isRecord(value) || !isVersion(value.version) || typeof value.id !== "string" || !isLane(value.lane)) return false;
  if (value.class !== "capability" && value.class !== "regression") return false;
  if (value.split !== "smoke" && value.split !== "regression" && value.split !== "held-out") return false;
  if (typeof value.inputArtifact !== "string" || !isHash(value.inputHash) || (value.scenarioContentHash !== undefined && !isHash(value.scenarioContentHash)) || typeof value.expectedOutcome !== "string") return false;
  if (!Array.isArray(value.deterministicExpectations) || !value.deterministicExpectations.every((item) => typeof item === "string" && item.length > 0)) return false;
  if (typeof value.rubricId !== "string" || (value.goodReference !== undefined && typeof value.goodReference !== "string") || (value.badReference !== undefined && typeof value.badReference !== "string")) return false;
...
export function isEvalRubric(value: unknown): value is EvalRubric {
  if (!isRecord(value) || !isVersion(value.version) || typeof value.id !== "string" || !isLane(value.lane) || !Array.isArray(value.dimensions) || value.dimensions.length === 0) return false;
  const ids = new Set<string>();
  return value.dimensions.every((dimension) => {
    if (!isRecord(dimension) || typeof dimension.id !== "string" || dimension.id.length === 0 || ids.has(dimension.id) || typeof dimension.description !== "string" || dimension.description.length === 0 || (dimension.blocking !== undefined && typeof dimension.blocking !== "boolean") || (dimension.minimumScore !== undefined && (typeof dimension.minimumScore !== "number" || dimension.minimumScore < 0 || dimension.minimumScore > 1))) return false;
    ids.add(dimension.id);
    return true;
  });
}

export function isEvalModelConfig(value: unknown): value is EvalModelConfig {
...
export function isEvalExperiment(value: unknown): value is EvalExperiment {
...
export function isGenerationRecord(value: unknown): value is GenerationRecord {
  if (!isRecord(value) || !isVersion(value.version) || value.kind !== "host-agent-generation") return false;
  if (typeof value.executionId !== "string" || value.executionId.length === 0 || typeof value.caseId !== "string" || !isLane(value.lane) || (value.role !== "baseline" && value.role !== "candidate")) return false;
  if (!isHash(value.inputHash) || !isHash(value.promptHash) || !isHash(value.outputHash) || typeof value.modelComparisonKey !== "string" || value.modelComparisonKey.length === 0) return false;
  if (!isRecord(value.producerContext) || value.producerContext.role !== "generator" || typeof value.producerContext.contextId !== "string" || value.producerContext.contextId.length === 0 || typeof value.producerContext.host !== "string" || value.producerContext.host.length === 0) return false;
  if (!isRecord(value.observed) || typeof value.observed.startedAt !== "string" || typeof value.observed.endedAt !== "string") return false;
  const usage = value.observed.usage;
  if (usage !== null && (!isRecord(usage) || !["inputTokens", "outputTokens", "totalTokens"].every((key) => usage[key] === null || typeof usage[key] === "number"))) return false;
...
export function isRoleExecutionRecord(value: unknown, role?: RoleExecutionRecord["role"]): value is RoleExecutionRecord {
  if (!isRecord(value) || !isVersion(value.version) || (value.kind !== "host-agent-quality-grade" && value.kind !== "host-agent-optimizer" && value.kind !== "host-agent-reviewer") || (value.role !== "quality-grader" && value.role !== "optimizer" && value.role !== "reviewer")) return false;
  if (role && value.role !== role) return false;
...
export function isEvalTrial(value: unknown): value is EvalTrial {
...
export function isAcceptedBaseline(value: unknown): value is AcceptedBaseline {
  if (!isRecord(value) || !isVersion(value.version) || typeof value.id !== "string" || typeof value.acceptedAt !== "string" || typeof value.productVersion !== "string" || typeof value.bundleVersion !== "string" || typeof value.auditSchemaVersion !== "number" || !isHash(value.runtimeHash) || !isEvalModelConfig(value.modelConfig) || !Array.isArray(value.caseIds) || !Array.isArray(value.trials) || !isRecord(value.knownVariance) || (value.budgetEvidencePolicy !== "required" && value.budgetEvidencePolicy !== "advisory") || (value.tokenMetric !== "output_tokens" && value.tokenMetric !== "total_tokens") || !Array.isArray(value.budgetLimitations) || !value.budgetLimitations.every((item) => typeof item === "string") || value.privacy !== "redacted") return false;
  const promptHashes = value.promptHashes;
  const caseIds = value.caseIds as unknown[];
  const baselineModelConfig = value.modelConfig as EvalModelConfig;
  if (!isRecord(promptHashes) || !isHash(promptHashes["report-synthesis"]) || !isHash(promptHashes["key-session-analysis"]) || !isHash(promptHashes["skill-insights"]) || !caseIds.every((id) => typeof id === "string" && id.length > 0) || caseIds.length === 0) return false;
  const bootstrap = value.bootstrap;
  if (bootstrap !== undefined) {
    if (!isRecord(bootstrap) || typeof bootstrap.previousBaselinePath !== "string" || !isHash(bootstrap.previousBaselineHash) || typeof bootstrap.invalidationReason !== "string" || bootstrap.invalidationReason.length === 0 || typeof bootstrap.invalidatedAt !== "string" || !isRecord(bootstrap.reviewHashes)) return false;
    const reviewHashes = bootstrap.reviewHashes;
    if (!isRecord(reviewHashes) || !["blindMapHash", "gradesHash", "varianceHash", "reviewEvidenceHash", "reviewerRecordHash", "qualityGraderRecordHash"].every((key) => isHash(reviewHashes[key]))) return false;
  }
  const seen = new Set<string>();
  return value.trials.length > 0 && value.trials.every((trial) => {
    if (!isEvalTrial(trial) || !hasAcceptedTrialEvidence(trial)) return false;
    if (seen.has(trial.id) || !caseIds.includes(trial.caseId)) return false;
    seen.add(trial.id);
...
export function evaluateBudget(input: { kind: BudgetKind; value: number | null; limit: number | null; policy: BudgetEvidencePolicy }): BudgetEvaluation {
  ...
  if (input.limit === null) return { ...input, observation: "unavailable", outcome: "inconclusive", errorCode: missingCode };
  ...
  if (input.value > input.limit) return { ...input, observation: "exceeded", outcome: "rejected", errorCode: exceededCode };
  return { ...input, observation: "within", outcome: "within", errorCode: null };
}

function aggregateBudget(values: readonly (number | null)[], kind: BudgetKind, limit: number | null, policy: BudgetEvidencePolicy): BudgetEvaluation {
  const evaluations = values.map((value) => evaluateBudget({ kind, value, limit, policy }));
  if (evaluations.some((item) => item.observation === "exceeded")) return evaluations.find((item) => item.observation === "exceeded") as BudgetEvaluation;
  if (evaluations.some((item) => item.observation === "unavailable")) return evaluations.find((item) => item.observation === "unavailable") as BudgetEvaluation;
  return evaluations[0] ?? evaluateBudget({ kind, value: null, limit, policy });
}

export function summarizeScores(trials: readonly EvalTrial[]): EvalBenchmarkSummary {
  const passed = trials.filter((trial) => trial.contractStatus === "passed").length;
  const dimensionIds = new Set<string>();
  for (const trial of trials) for (const id of Object.keys(trial.quality)) dimensionIds.add(id);
  const dimensions: EvalBenchmarkSummary["dimensions"] = {};
  for (const id of dimensionIds) {
    const values = trials.map((trial) => trial.quality[id]).filter((value): value is number => typeof value === "number");
  ...
  const numeric = (key: "durationMs" | "tokenCount"): number[] => trials.map((trial) => trial[key]).filter((value): value is number => typeof value === "number");
...
const REQUIRED_DIMENSIONS: Record<EvalLane, string[]> = {
...
function hasRealTrialEvidence(trial: EvalTrial): boolean {
  return Boolean(
    isHash(trial.outputHash)
...
function hasAcceptedTrialEvidence(trial: EvalTrial): boolean {
  return Boolean(
    hasRealTrialEvidence(trial)
    && Object.values(trial.quality).length > 0
    && Object.values(trial.quality).every((score) => typeof score === "number" && score >= 0 && score <= 1),
  );
}

function trialShapeErrors(trials: readonly EvalTrial[], role: EvalTrial["role"], experiment: EvalExperiment): string[] {
  const errors: string[] = [];
  for (const trial of trials) {
    if (trial.role !== role) errors.push("PROMOTION_TRIAL_ROLE_MISMATCH");
    if (!isHash(trial.promptHash)) errors.push("PROMOTION_PROMPT_HASH_INVALID");
    if (role === "baseline" && trial.promptHash !== experiment.baselinePromptHash) errors.push("PROMOTION_BASELINE_PROMPT_HASH_MISMATCH");
    if (role === "candidate" && trial.promptHash !== experiment.candidatePromptHash) errors.push("PROMOTION_CANDIDATE_PROMPT_HASH_MISMATCH");
    if (isGenerationRecord(trial.generationRecord) && trial.generationRecord.codexProvenance.tokenMetric !== experiment.tokenMetric) errors.push("PROMOTION_TOKEN_METRIC_MISMATCH");
    if (!hasRealTrialEvidence(trial)) errors.push("PROMOTION_TRIAL_EVIDENCE_MISSING");
  }
  return errors;
}

function dimensionErrors(trials: readonly EvalTrial[], rubric: EvalRubric, prefix: string): string[] {
  const errors: string[] = [];
  ...
  for (const trial of trials) {
    for (const dimension of rubric.dimensions) {
      ...
      if (score === undefined) errors.push(`${prefix}_RUBRIC_DIMENSION_MISSING:${dimension.id}`);
      else if (score === "unknown") errors.push(`${prefix}_RUBRIC_DIMENSION_UNKNOWN:${dimension.id}`);
      else if (typeof score !== "number" || score < (dimension.minimumScore ?? 0) || score > 1) errors.push(`${prefix}_RUBRIC_DIMENSION_BELOW_MINIMUM:${dimension.id}`);
      const evidence = trial.qualityEvidence?.[dimension.id];
      if (!Array.isArray(evidence) || evidence.length === 0) errors.push(`${prefix}_RUBRIC_EVIDENCE_MISSING:${dimension.id}`);
    }
    for (const id of Object.keys(trial.quality)) if (!required.has(id)) errors.push(`${prefix}_RUBRIC_DIMENSION_UNDECLARED:${id}`);
  }
  return errors;
}

function mustNotRegressErrors(
  experiment: EvalExperiment,
  baselineSummary: EvalBenchmarkSummary,
  candidateSummary: EvalBenchmarkSummary,
): string[] {
  const errors: string[] = [];
...
function coverageErrors(
  experiment: EvalExperiment,
  cases: readonly EvalCase[],
  baseline: readonly EvalTrial[],
  candidate: readonly EvalTrial[],
  minimumTrials: number,
): string[] {
  const errors: string[] = [];
  const caseMap = new Map(cases.map((item) => [item.id, item]));
  const expectedIds = unique(experiment.caseIds);
  if (expectedIds.length !== experiment.caseIds.length) errors.push("PROMOTION_DUPLICATE_CASE_ID");
  for (const caseId of expectedIds) {
    ...
    if (evalCase.lane !== experiment.lane) errors.push(`PROMOTION_CASE_LANE_MISMATCH:${caseId}`);
    const base = baseline.filter((trial) => trial.caseId === caseId);
    const cand = candidate.filter((trial) => trial.caseId === caseId);
    if (base.length < minimumTrials) errors.push(`PROMOTION_BASELINE_TRIALS_MISSING:${caseId}`);
    if (cand.length < minimumTrials) errors.push(`PROMOTION_CANDIDATE_TRIALS_MISSING:${caseId}`);
    for (const trial of [...base, ...cand]) {
      if (trial.inputHash !== evalCase.inputHash) errors.push(`PROMOTION_INPUT_HASH_MISMATCH:${caseId}`);
    }
    if (evalCase.split === "held-out" && cand.length < minimumTrials) errors.push(`PROMOTION_HELD_OUT_TRIAL_MISSING:${caseId}`);
  }
  for (const trial of [...baseline, ...candidate]) if (!expectedIds.includes(trial.caseId)) errors.push(`PROMOTION_UNDECLARED_CASE:${trial.caseId}`);
  ...
  for (const item of cases.filter((item) => item.split === "held-out")) {
    if (developmentInputs.has(item.inputHash)) errors.push(`PROMOTION_HELD_OUT_INPUT_NOT_DISTINCT:${item.id}`);
  }
  return errors;
}

function generationUniquenessErrors(trials: readonly EvalTrial[], prefix: string): string[] {
  const errors: string[] = [];
  ...
  for (const trial of trials) {
    ...
    if (!record) continue;
    if (executions.has(record.executionId)) errors.push(`${prefix}_GENERATION_RECORD_DUPLICATE:${record.executionId}`);
    executions.add(record.executionId);
    if (sources.has(record.sourceReference)) errors.push(`${prefix}_GENERATION_SOURCE_DUPLICATE:${record.sourceReference}`);
    sources.add(record.sourceReference);
  }
  return errors;
}

function baselineErrors(experiment: EvalExperiment, accepted: AcceptedBaseline, baseline: readonly EvalTrial[]): string[] {
  const errors: string[] = [];
  const acceptedHash = accepted.promptHashes[experiment.lane];
  if (!isHash(acceptedHash) || acceptedHash !== experiment.baselinePromptHash) errors.push("PROMOTION_ACCEPTED_BASELINE_HASH_MISMATCH");
  if (experiment.baselinePromptHash === "0".repeat(64)) errors.push("PROMOTION_BASELINE_HASH_ZERO");
  if (experiment.candidatePromptHash === experiment.baselinePromptHash) errors.push("PROMOTION_PROMPT_HASH_UNCHANGED");
  if (!isHash(experiment.candidatePromptHash)) errors.push("PROMOTION_CANDIDATE_HASH_INVALID");
  const acceptedByCase = new Map<string, EvalTrial[]>();
...
function comparabilityErrors(experiment: EvalExperiment, baseline: readonly EvalTrial[], candidate: readonly EvalTrial[]): string[] {
  const errors: string[] = [];
  const sameConfig = (trial: EvalTrial): boolean => JSON.stringify(trial.modelConfig) === JSON.stringify(experiment.modelConfig);
  if (baseline.some((trial) => !sameConfig(trial))) errors.push("PROMOTION_BASELINE_MODEL_CONFIG_MISMATCH");
  if (candidate.some((trial) => !sameConfig(trial))) errors.push("PROMOTION_CANDIDATE_MODEL_CONFIG_MISMATCH");
  ...
  if (baseKeys.size !== 1 || !baseKeys.has(experiment.modelConfig.comparisonKey)) errors.push("PROMOTION_BASELINE_MODEL_NOT_COMPARABLE");
  if (candidateKeys.size !== 1 || !candidateKeys.has(experiment.modelConfig.comparisonKey)) errors.push("PROMOTION_CANDIDATE_MODEL_NOT_COMPARABLE");
  if (baseKeys.size === 1 && candidateKeys.size === 1 && [...baseKeys][0] !== [...candidateKeys][0]) errors.push("PROMOTION_MODEL_COMPARISON_KEY_MISMATCH");
  return errors;
}

function outputDifferenceErrors(baseline: readonly EvalTrial[], candidate: readonly EvalTrial[]): string[] {
  const errors: string[] = [];
  const baseByCase = new Map<string, Set<string>>();
  const candidateByCase = new Map<string, Set<string>>();
  for (const trial of baseline) if (trial.outputHash) (baseByCase.get(trial.caseId) ?? baseByCase.set(trial.caseId, new Set()).get(trial.caseId)!).add(trial.outputHash);
  for (const trial of candidate) if (trial.outputHash) (candidateByCase.get(trial.caseId) ?? candidateByCase.set(trial.caseId, new Set()).get(trial.caseId)!).add(trial.outputHash);
  ...
  if (!changed) errors.push("PROMOTION_NO_OUTPUT_DIFFERENCE");
  return errors;
}
...
export function reviewExperiment(
  ...
  gate?: PromotionGateInput,
): EvalReview {
  const minimumTrials = PROMOTION_MINIMUM_TRIALS;
  ...
  const gateErrors = gate
    ? [
      ...
      ...gate.cases.filter((item) => item.id && item.lane === experiment.lane).flatMap((item) => {
        ...
        if (!rubric) return [`PROMOTION_RUBRIC_MISSING:${item.rubricId}`];
  ...
  return {
    ...
    budgetLimitations: [tokenBudget, timeBudget].filter((item) => item.observation === "unavailable" && item.errorCode).map((item) => item.errorCode as string),
    createdAt: new Date().toISOString(),
  };
}

export function reviewArtifactBindingHash(artifact: Record<string, unknown>): string {
...
export function makeOptimizerProposal(
  revision: number,
  hypothesis: string,
  failedDimensions: string[],
  evidence: string[],
): OptimizerProposal {
  if (!Number.isInteger(revision) || revision < 1 || revision > 3) throw new Error("Optimizer revisions are bounded to one through three.");
