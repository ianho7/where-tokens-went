...
import type {
  ...
  SkillInsightReveal,
  SkillInsightRevealPattern,
  MentalModelShift,
  DecisionDelta,
  SkillFamilyUsage,
  ValidatedSkillInsight,
  SkillInsightKind,
  SkillInsightClaimStrength,
  SkillInsightRejectionReason,
  SkillInsightCounterfactual,
  SkillContentProfile,
} from "./types";
...
export function calculateSkillSnapshotId(snapshot: Omit<SkillSnapshotArtifact, "snapshotId">): string {
  const identity = {
...
const ALLOWED_INSIGHT_KINDS = new Set<SkillInsightKind>(["usage", "capability", "mechanism"]);
const ALLOWED_CLAIM_STRENGTHS = new Set<SkillInsightClaimStrength>(["coexistence", "scaffold-interpretation", "primary-delta"]);
export const SKILL_INSIGHT_REJECTION_REASONS: readonly SkillInsightRejectionReason[] = [
...
const ALLOWED_CANDIDATE_TYPES = new Set<SkillCandidateType>([
  "high_frequency",
  "high_calls_per_task",
  "skill_family",
...
export function skillInsightEvidenceReference(evidence: SkillInsightEvidence): string {
  if (evidence.kind === "global_metric") return `global:${evidence.metric ?? ""}`;
  if (evidence.kind === "distribution_metric") return `distribution:${evidence.metric ?? ""}`;
  if (evidence.kind === "family_metric") return `family:${evidence.familyId ?? ""}:${evidence.metric ?? ""}`;
  if (evidence.kind === "skill_metric") return `skill:${evidence.skillId ?? ""}:${evidence.metric ?? ""}`;
...
export function skillInsightEvidenceMatchesReference(evidence: SkillInsightEvidence, reference: string): boolean {
  if (skillInsightEvidenceReference(evidence) === reference) return true;
  return (evidence.kind === "skill_content" || evidence.kind === "cross_skill_content") && reference === `content:${evidence.skillId ?? ""}`;
}

function parseReveal(value: unknown): SkillInsightReveal | undefined {
  if (!value || typeof value !== "object") return undefined;
  const raw = value as Record<string, unknown>;
  const semantic = typeof raw.semantic === "string" ? raw.semantic.trim() : "";
  const pattern = typeof raw.pattern === "string" ? raw.pattern.trim() as SkillInsightRevealPattern : undefined;
  const evidenceRefs = Array.isArray(raw.evidenceRefs)
    ? raw.evidenceRefs.filter((ref): ref is string => typeof ref === "string").map((ref) => ref.trim()).filter(Boolean)
    : [];
  if (!semantic || !pattern || !ALLOWED_REVEAL_PATTERNS.has(pattern) || evidenceRefs.length === 0) return undefined;
  return { semantic, pattern, evidenceRefs };
}

function parseCounterfactual(value: unknown): SkillInsightCounterfactual | undefined {
  if (!value || typeof value !== "object") return undefined;
  const raw = value as Record<string, unknown>;
  ...
  if (!ifRemoved || !withoutGenericScaffold) return undefined;
  return { ifRemoved, withoutGenericScaffold };
}

function isVerifiedExcerpt(snapshotSkillsMap: Map<string, SkillContentSnapshot>, skillId: string, excerpt: string): boolean {
  const skillSnapshot = snapshotSkillsMap.get(skillId);
  if (!skillSnapshot || skillSnapshot.contentState !== "available" || !skillSnapshot.skillMdContent) return false;
  return normalizeText(skillSnapshot.skillMdContent).includes(normalizeText(excerpt));
}

function parseSkillContentProfiles(value: unknown, snapshotSkillsMap: Map<string, SkillContentSnapshot>): SkillContentProfile[] {
  ...
  for (const item of value) {
    ...
    if (!skillId || !contentSummary || !snapshotSkillsMap.has(skillId)) continue;

    const lossIfRemoved = Array.isArray(raw.lossIfRemoved)
      ? raw.lossIfRemoved.flatMap((entry): SkillContentProfile["lossIfRemoved"] => {
          ...
          const loadingScope = typeof candidate.loadingScope === "string" && ALLOWED_LOADING_SCOPES.has(candidate.loadingScope as SkillLoadingScope)
            ...
            : undefined;
          if (!summary || !role || !evidenceExcerpt || evidenceExcerpt.length > 200 || !whyModelWouldNotKnowThis || !loadingScope || !isVerifiedExcerpt(snapshotSkillsMap, skillId, evidenceExcerpt)) return [];
    ...
    const modelNativeScaffold = Array.isArray(raw.modelNativeScaffold)
      ? raw.modelNativeScaffold.flatMap((entry): SkillContentProfile["modelNativeScaffold"] => {
          if (!entry || typeof entry !== "object") return [];
          const candidate = entry as Record<string, unknown>;
          const summary = typeof candidate.summary === "string" ? candidate.summary.trim() : "";
          const evidenceExcerpt = typeof candidate.evidenceExcerpt === "string" ? candidate.evidenceExcerpt.trim() : "";
          ...
          if (!summary || !evidenceExcerpt || evidenceExcerpt.length > 200 || !observed || !interpretation || !rationale || !relativeTo || !isVerifiedExcerpt(snapshotSkillsMap, skillId, evidenceExcerpt)) return [];
    ...
    const scopedContent = Array.isArray(raw.scopedContent)
      ? raw.scopedContent.flatMap((entry): SkillContentProfile["scopedContent"] => {
          if (!entry || typeof entry !== "object") return [];
          const candidate = entry as Record<string, unknown>;
          const summary = typeof candidate.summary === "string" ? candidate.summary.trim() : "";
          const activationCondition = typeof candidate.activationCondition === "string" ? candidate.activationCondition.trim() : "";
          const evidenceExcerpt = typeof candidate.evidenceExcerpt === "string" ? candidate.evidenceExcerpt.trim() : "";
          if (!summary || !activationCondition || !evidenceExcerpt || evidenceExcerpt.length > 200 || !isVerifiedExcerpt(snapshotSkillsMap, skillId, evidenceExcerpt)) return [];
...
function hasEvidenceRef(insight: SkillInsightReveal, evidence: SkillInsightEvidence[], reference: string): boolean {
  return insight.evidenceRefs.includes(reference) && evidence.some((entry) => skillInsightEvidenceMatchesReference(entry, reference));
}

function hasMetricEvidence(evidence: SkillInsightEvidence[], kind: SkillInsightEvidenceKind, metric: string): boolean {
  return evidence.some((entry) => entry.kind === kind && entry.metric === metric);
}

function validateRevealEvidenceContract(
  id: string,
...
export function validateSkillInsights(
  ...
  const rawRecord = !Array.isArray(raw) ? raw as Record<string, unknown> : null;
  ...
  const addRejectionReason = (reason: SkillInsightRejectionReason): void => {
  ...
  for (const item of rawList) {
    ...
    const rawKind = typeof c.kind === "string" ? c.kind.trim() as SkillInsightKind : undefined;
    const kind: SkillInsightKind = rawKind ?? (reveal?.pattern === "content_contrast" ? "capability" : "usage");
    const candidateType = typeof c.candidateType === "string" && ALLOWED_CANDIDATE_TYPES.has(c.candidateType as SkillCandidateType)
      ? c.candidateType as SkillCandidateType
      : undefined;
    const rawClaimStrength = typeof c.claimStrength === "string" ? c.claimStrength.trim() as SkillInsightClaimStrength : undefined;
    ...
    const familyDifferences = Array.isArray(c.familyDifferences)
      ? c.familyDifferences.filter((value): value is string => typeof value === "string").map((value) => value.trim()).filter(Boolean).slice(0, 4)
      : [];
    if (kind === "capability") capabilityCandidateSeen = true;
  ...
  const rankedInsights = [...acceptedInsights].sort((a, b) => {
    const kindRank = (value: SkillInsightKind): number => value === "capability" ? 3 : value === "mechanism" ? 2 : 1;
  ...
  const mentalModelKeys = new Set<string>();
  ...
  if (readableSkillCount > 0 && !finalInsights.some((insight) => insight.kind === "capability")) {
    if (!capabilityCandidateSeen) addRejectionReason("usage_content_relation_unclear");
