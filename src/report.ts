...
import { skillInsightEvidenceMatchesReference } from "./skill-insights";
...
export interface ReportFontConfig {
  filePath: string;
  family?: string;
}
...
function formatCurrency(value: number | string | null, locale: ReportLocale): string {
  if (value === null) return labelsFor(locale).unavailable;
...
type MetricKind = "metric" | "currency" | "percentage";

function metricValueHtml(value: EvidenceValue, locale: ReportLocale, kind: MetricKind, compact = true): string {
  return kind === "percentage"
    ? percentageHtml(value, locale)
    : evidenceHtml(value, locale, compact, kind === "currency");
}

function renderMetric(label: string, value: EvidenceValue, locale: ReportLocale, kind: MetricKind = "metric", compact = true): string {
...
function renderMetrics(items: Array<[string, EvidenceValue, MetricKind, boolean?]>, locale: ReportLocale, variant = "default"): string {
  const countClass = items.length > 0 ? " metrics--count-" + items.length : "";
  return "<div class=\"metrics metrics--" + variant + countClass + "\">" + items.map(([label, value, kind, compact]) =>
    renderMetric(label, value, locale, kind, compact ?? true),
  ).join("") + "</div>";
}

function tagHtml(label: string, variant: "quiet" | "default" = "quiet"): string {
...
function renderCoverage(result: AuditResult, locale: ReportLocale): string {
  ...
  const metrics: Array<[string, EvidenceValue, MetricKind, boolean]> = [
...
function reportDateRange(result: AuditResult, locale: ReportLocale): string {
  const dates = result.report.dailyUsage.map((row) => row.key).filter((value) => /^\d{4}-\d{2}-\d{2}$/.test(value));
  if (dates.length === 0) return formatDateTime(result.scope.since, locale).split(" ")[0].slice(5);
  const first = formatDateKey(dates[0], locale, false);
  const last = formatDateKey(dates[dates.length - 1], locale, false);
  return first === last ? first : first + "—" + last;
}

function reportEndDate(result: AuditResult, locale: ReportLocale): string {
  const dates = result.report.dailyUsage.map((row) => row.key).filter((value) => /^\d{4}-\d{2}-\d{2}$/.test(value));
...
export function resolveReportProjectName(cwd: string | null): string | null {
...
function isTokenAccountingWarning(warning: string): boolean {
...
function topTaskTokenAccountingStatus(result: AuditResult): "reconciled" | "mismatch" | "unavailable" | null {
...
function renderKpis(result: AuditResult, locale: ReportLocale): string {
  ...
  const sessionBreakdown = result.summary.topLevelSessionCount.value !== null && result.summary.subagentSessionCount.value !== null
    ? [[labels.topLevelSessions, result.summary.topLevelSessionCount, "metric"], [labels.subagentSessions, result.summary.subagentSessionCount, "metric"]] as Array<[string, EvidenceValue, MetricKind]>
    : [];
  const items: Array<[string, EvidenceValue, MetricKind]> = [
    ...sessionBreakdown,
    ...(result.summary.reportedCost.value === null ? [] : [[labels.reportedCost, result.summary.reportedCost, "currency"] as [string, EvidenceValue, MetricKind]]),
  ];
...
function checkName(checkId: string, locale: ReportLocale): string {
  const labels = labelsFor(locale);
  if (checkId === "long_session") return labels.kindLongSession;
  if (checkId === "tool_amplification") return labels.kindToolAmplification;
  if (checkId === "extra_calls") return labels.kindExtraCalls;
...
function reportFindingEvidence(result: AuditResult, reference: string, locale: ReportLocale): string {
  ...
  if (!match) return labels.unavailable;
  if (match.kind === "check") {
...
function sessionTurns(result: AuditResult, sessionId: string): TurnAnalysisEntry[] {
  return (result.turns ?? []).filter((turn) => turn.sessionId === sessionId);
}

function sessionActiveTime(result: AuditResult, sessionId: string): EvidenceValue {
  const turns = sessionTurns(result, sessionId);
  const durations = turns.map((turn) => turn.durationMs.value).filter((value): value is number => typeof value === "number");
...
function sessionCompleteness(result: AuditResult, sessionId: string): EvidenceValue {
  const turns = sessionTurns(result, sessionId);
  if (turns.length === 0) return { value: null, provenance: "unavailable", method: "Session has no Turn trajectory" };
...
function sessionTitle(row: ContributionEntry, locale: ReportLocale): string {
...
function keyMetricHtml(value: EvidenceValue, locale: ReportLocale, className = "", compact = true): string {
  const classes = className ? " " + className : "";
  if (value.value === null) return "<span class=\"key-value unavailable" + classes + "\" aria-label=\"" + escapeHtml(labelsFor(locale).unavailable) + "\">—</span>";
...
function keyPercentageHtml(value: EvidenceValue, locale: ReportLocale, className = ""): string {
  if (value.value === null) return keyMetricHtml(value, locale, className, false);
...
function durationText(value: EvidenceValue, locale: ReportLocale): string {
...
function fixedPercent(value: number, locale: ReportLocale): string {
  return new Intl.NumberFormat(locale, { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(value) + "%";
}

function concentrationText(turns: TurnAnalysisEntry[], sessionTotal: EvidenceValue, locale: ReportLocale): string {
...
function renderTurnTrajectory(result: AuditResult, sessionId: string, locale: ReportLocale): string {
  const labels = labelsFor(locale);
  const turns = sessionTurns(result, sessionId);
  if (turns.length === 0) return emptyState(labels, labels.noTrajectory);
...
function renderKeySessionAnalysis(result: AuditResult, locale: ReportLocale, composition?: ReportComposition, localFirstUserMessages?: FirstUserMessageRecord[]): string {
  ...
  const blocks = topSessions.map((row, index) => {
    ...
    const decisionState: "testable" | "not_recommended" | "unavailable" = !analysis
      ? "unavailable"
...
function renderSessions(result: AuditResult, locale: ReportLocale): string {
  if ((result.turns ?? []).length > 0) return renderExplainableSessions(result, locale);
  const labels = labelsFor(locale);
  const rows = result.rankings.sessions.slice(0, 10);
  if (rows.length === 0) return emptyState(labels);
...
function renderCacheHtml(result: AuditResult, locale: ReportLocale): string {
  const labels = labelsFor(locale);
  const cache = result.report.cacheEconomics;
  const efficiencyMetrics: Array<[string, EvidenceValue, MetricKind]> = [
  ...
  const impactMetrics: Array<[string, EvidenceValue, MetricKind]> = [
...
function renderFirstRequestHtml(result: AuditResult, locale: ReportLocale): string {
  const labels = labelsFor(locale);
  const first = result.report.firstRequestBurden;
  const metrics: Array<[string, EvidenceValue, MetricKind]> = [
...
function skillMetricLabel(metric: string | undefined, locale: ReportLocale): string {
  ...
  return metric && metric in labels ? labels[metric as keyof typeof labels] : locale === "zh-CN" ? "证据" : "Evidence";
}

function formatSkillMetricValue(metric: string | undefined, value: number | string | undefined, locale: ReportLocale): string {
...
function revealEvidence(item: ValidatedSkillInsight): SkillInsightEvidence[] {
  return item.reveal.evidenceRefs
    .map((reference) => item.evidence.find((entry) => skillInsightEvidenceMatchesReference(entry, reference)))
    .filter((entry): entry is SkillInsightEvidence => Boolean(entry));
}

function revealMetric(item: ValidatedSkillInsight, kind: SkillInsightEvidence["kind"], metric: string, skillId?: string): SkillInsightEvidence | undefined {
  return item.evidence.find((entry) => entry.kind === kind && entry.metric === metric && (!skillId || entry.skillId === skillId));
}

function renderSkillInsightReveal(item: ValidatedSkillInsight, locale: ReportLocale): string {
...
function renderSkillInsightProof(item: ValidatedSkillInsight, locale: ReportLocale): string {
...
interface HourlyChartData {
  ...
  cells: HourlyChartCell[];
  max: number;
}

function hourlyChartData(result: AuditResult, locale: ReportLocale): HourlyChartData {
...
function renderHourly(result: AuditResult, locale: ReportLocale): string {
  ...
  if (data.dates.length === 0) return emptyState(labels, labels.noTimestampData);
...
let cachedChartRuntime: string | null = null;
function chartRuntime(): string {
  if (cachedChartRuntime !== null) return cachedChartRuntime;
  try {
    cachedChartRuntime = readFileSync(join(__dirname, "assets", "echarts.min.js"), "utf8");
  } catch {
    try {
      cachedChartRuntime = readFileSync(join(__dirname, "..", "assets", "echarts.min.js"), "utf8");
    } catch {
...
const fontDataCache = new Map<string, FontAsset>();
function fontAsset(filePath: string): FontAsset | null {
  const location = resolvePath(filePath);
  ...
  try {
    ...
    const dataUrl = "data:" + metadata.mime + ";base64," + readFileSync(location).toString("base64");
...
function fontFaceMarkup(comment: string, family: string, asset: FontAsset, weight: number): string {
...
function bundledFontAsset(fileName: string): FontAsset | null {
  const locations = [join(__dirname, "assets", "fonts", fileName), join(__dirname, "..", "assets", "fonts", fileName)];
...
let cachedBundledFontFaces: { css: string; family: string | null } | null = null;

function authorizedFontFaces(fontConfig?: ReportFontConfig): { css: string; family: string | null } {
  if (fontConfig) {
...
function configuredFontVariables(family: string): string {
...
export function collectReportFontCharacters(html: string): string {
  const characters = new Set<string>();
  for (const character of Array.from(html.replace(FONT_DATA_URL_PATTERN, ""))) characters.add(character);
  for (const character of Array.from(REQUIRED_FONT_CHARACTERS)) characters.add(character);
  return [...characters].join("");
}

function warnFontSubsettingFailure(): void {
...
function loadCachedFontEntries(fontFingerprint: string, weight: string): CachedFontEntry[] {
  const cacheDir = getFontCacheDir();
  const entries: CachedFontEntry[] = [];
  try {
    const files = readdirSync(cacheDir);
    const prefix = fontFingerprint + "-" + weight + "-";
    for (const file of files) {
      if (file.startsWith(prefix) && file.endsWith(".json")) {
        const jsonPath = join(cacheDir, file);
        const woff2Path = join(cacheDir, file.replace(/\.json$/, ".woff2"));
        if (!existsSync(woff2Path)) continue;
        const meta = JSON.parse(readFileSync(jsonPath, "utf8"));
        if (typeof meta.chars === "string") {
          const charSet = new Set(meta.chars as string);
          entries.push({
            file: woff2Path,
            charSet,
...
function findSupersetFont(entries: CachedFontEntry[], requiredCharacters: string): Buffer | null {
...
function saveCachedFont(fontFingerprint: string, weight: string, chars: string, woff2Buffer: Buffer): void {
  const cacheDir = getFontCacheDir();
  try {
...
export async function subsetReportFonts(html: string): Promise<string> {
  ...
  if (sources.length !== 2 || weights.size !== 2 || !weights.has("400") || !weights.has("500")) return html;

  try {
    const subsetFont = require("subset-font") as (input: Buffer, text: string, options: { targetFormat: "woff2" }) => Promise<Buffer>;
    ...
    for (const source of sources) {
      ...
      const fontFingerprint = createHash("sha256").update(input).digest("hex").slice(0, 12);
      ...
      if (!subset) {
        ...
        if (subset.length < 4 || subset.subarray(0, 4).toString("ascii") !== "wOF2") throw new Error("The subset font was not WOFF2.");
...
function keySessionChartData(result: AuditResult, locale: ReportLocale, firstUserMessages: FirstUserMessageRecord[]): Array<{
  chartId: string;
  turns: Array<{
    n: number | null;
    label: string;
...
function keyChartScript(locale: ReportLocale): string {
  const labels = labelsFor(locale);
...
function renderInteractiveCharts(result: AuditResult, locale: ReportLocale, firstUserMessages: FirstUserMessageRecord[] = []): string {
  const labels = labelsFor(locale);
  ...
  if (!runtime || (rows.length === 0 && hourly.dates.length === 0 && !keySessions.some((session) => session.turns.length > 0))) return "";
...
function readStylesheet(): string {
  ...
  try {
    cachedStylesheet = readFileSync(join(__dirname, "report.css"), "utf8");
  } catch {
    try {
      cachedStylesheet = readFileSync(join(__dirname, "..", "report.css"), "utf8");
    } catch {
...
function renderStyles(fontConfig?: ReportFontConfig): string {
