...
function fontDataUrl(fileName: string): string {
  ...
  for (const location of locations) {
    try {
      const dataUrl = "data:font/ttf;base64," + readFileSync(location).toString("base64");
...
function authorizedFontFaces(): string {
...
function renderStyles(): string {
  const stylesheet = readFileSync(join(__dirname, "report.css"), "utf8");
  return "<style>" + authorizedFontFaces() + "\n" + stylesheet + "</style>";
}
export function renderHtml(result: AuditResult, locale: ReportLocale = "en-US", composition?: ReportComposition, localFirstUserMessages?: FirstUserMessageRecord[]): string {
  const labels = labelsFor(locale);
