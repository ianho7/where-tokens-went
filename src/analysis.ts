...
function buildSessionTotalsIndex(calls: ModelCallRecord[], harness: Harness): Map<string, number | null> {
  const totals = new Map<string, { sum: number; complete: boolean; count: number }>();
...
interface ToolAmplificationIndex {
  laterCallsForTool: (tool: ToolCallRecord) => number;
}

function buildToolAmplificationIndex(
  calls: ModelCallRecord[],
  lifecycle: ReadResult["lifecycle"],
): ToolAmplificationIndex {
  const compactionTimesBySession = new Map<string, number[]>();
  ...
  const callTimesBySession = new Map<string, number[]>();
  for (const call of calls) {
    if (call.activeBranch === false || !call.timestamp) continue;
    const time = timestampMs(call.timestamp);
    if (time !== null) {
  ...
  const memo = new Map<ToolCallRecord, number>();

  function laterCallsForTool(tool: ToolCallRecord): number {
    ...
    let nextCompaction: number | undefined;
...
function buildToolAnalysis(read: ReadResult, amplificationIndex?: ToolAmplificationIndex): { entries: ToolAnalysisEntry[]; total: EvidenceValue } {
...
function buildTurnAnalysis(
  read: ReadResult,
  totalTokens: number | null,
  harness: Harness,
  precomputedSessionTotals?: Map<string, number | null>,
): {
  ...
  const callsWithoutTurnBySession = new Map<string, ModelCallRecord[]>();
  ...
  const toolsByTurn = new Map<string, ToolCallRecord[]>();
  for (const tool of read.toolCalls) {
    if (tool.turnId) {
  ...
  const lifecycleByTurn = new Map<string, LifecycleRecord[]>();
  ...
  const skillEvidenceByTurn = new Map<string, Set<string>>();
  ...
  const turnsBySession = new Map<string, TurnAnalysisEntry[]>();
...
function buildReportData(
  ...
  totalTokens: number | null,
  harness: Harness,
  pricing: ApiPricingContext,
  amplificationIndex?: ToolAmplificationIndex,
): ReportData {
...
function topSession(
  ...
  calls: ModelCallRecord[],
  harness: Harness,
  sessionTotals?: Map<string, number | null>,
): { session: SessionRecord; tokens: number } | null {
  const callsCountBySession = new Map<string, number>();
  for (const call of calls) {
